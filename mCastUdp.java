...
public class mCastUdp {
	...
	mCastListenPacket mcastlisner;
	public void startListener()
	{
		try{
			mcastlisner = new mCastListenPacket(soc);
			Thread lisner_thread = new Thread(mcastlisner);
	...
	public void zahyouGet(double x, double y, int port) {
...
//ListenPacketクラス
//マルチキャストパケットを受信
class mCastListenPacket implements Runnable {
	MulticastSocket s = null;//初期化
	// コンストラクタマルチキャストスレッド受け取り
	public mCastListenPacket(MulticastSocket soc){
		s = soc;
	}
